...
# Load Rules & Drug Intelligence Dataset Safely
@st.cache_data
def load_drug_database():
    try:
        df = pd.read_csv("drug_rules.csv")
        # Ensure column mapping consistency
        col_map = {
            "Molecular_Weight": "MW",
            "MolecularWeight": "MW",
            "Half_Life": "Half_Life_hrs",
            "HalfLife": "Half_Life_hrs",
            "BCS": "BCS_Class",
            "Dose": "Dose_mg"
        }
...
if tabs == "1. Drug Intelligence":
    ...
    drug_col = "Drug_Name" if "Drug_Name" in df_drugs.columns else df_drugs.columns[0]
    selected_drug = st.selectbox("Select API:", df_drugs[drug_col].unique())
    drug_data = df_drugs[df_drugs[drug_col] == selected_drug].iloc[0]

    # Safe Key Retriever Function
    ...
    mw = get_val(["MW", "Molecular_Weight", "MolecularWeight"])
    logp = get_val(["LogP", "logP"])
    bcs = get_val(["BCS_Class", "BCS", "BCS_class"])
    thalf = get_val(["Half_Life_hrs", "Half_Life", "HalfLife"], default=4.0)
    dose = get_val(["Dose_mg", "Dose", "Dose_Mg"], default=250)
...
elif tabs == "3. Multi-Design Formulation Optimizer":
